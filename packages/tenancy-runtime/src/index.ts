...
export class TenancyStore {
  ...
  listOrganizations(): Organization[] {
    return this.db
      ...
      `)
      .all()
      .map((row: any) => this.mapOrganization(row));
  }
  ...
  listMembers(organizationId: string): OrganizationMember[] {
    ...
    return rows.map((row) => ({
      ...
      appIds: (accessStatement.all(row.id) as any[]).map(
        (entry) => String(entry.app_id)
      ),
      createdAt: row.created_at
    }));
  }
  ...
  listAudit(
    ...
    return this.db
      .prepare(`
        ...
        LIMIT ?
      `)
      .all(organizationId, Math.max(1, Math.min(limit, 500)))
      .map((row: any) => ({
        id: row.id,
  ...
  private requireMember(memberId: string): OrganizationMember {
    const member = this.listMembersById(memberId);
