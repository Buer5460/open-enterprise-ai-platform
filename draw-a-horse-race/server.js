...
io.on("connection",socket=>{
  socket.on("room:create",({userId,playerName,horse},ack=()=>{})=>{
    if(!userId)return ack({ok:false,message:"用户标识缺失"});
  ...
  socket.on("room:join",({code,userId,playerName,horse},ack=()=>{})=>{
    const room=rooms.get(String(code));if(!room)return ack({ok:false,message:"房间不存在或已过期"});
    let player=findPlayer(room,userId);
    if(room.status!=="lobby"&&!player)return ack({ok:false,message:"比赛已经开始，不能新增参赛者"});
    if(!player){if(room.players.length>=8)return ack({ok:false,message:"房间已满"});const cleanHorse=sanitizeHorse(horse),name=String(playerName||cleanHorse.name||"参赛者").slice(0,12);player={id:userId,socketId:socket.id,name,horse:cleanHorse,ready:true,bet:null};room.players.push(player);updateUser(userId,{name,horse:cleanHorse});}
    else player.socketId=socket.id;
    socket.join(room.code);emitRoom(room);ack({ok:true,room:publicRoom(room),race:room.status==="racing"?room.race:null});
    if(room.status==="racing"&&room.race)socket.emit("race:plan",{race:room.race,players:publicRoom(room).players,recovery:true});
  });

  socket.on("room:add-bot",({code,userId},ack=()=>{})=>{
    const room=rooms.get(String(code));if(!room)return ack({ok:false,message:"房间不存在"});if(room.hostId!==userId)return ack({ok:false,message:"只有房主可以添加试玩马"});if(room.status!=="lobby")return ack({ok:false,message:"当前不能添加"});if(room.players.length>=8)return ack({ok:false,message:"房间已满"});
    const botNames=["马界毕加索","风一样的胖子","不会画马","闪电萝卜","今天要夺冠","这不是狗"];const idx=room.players.filter(p=>p.isBot).length;
    const id=`bot_${room.code}_${Date.now()}_${idx}`;const name=botNames[idx%botNames.length];
    room.players.push({id,socketId:null,name,isBot:true,horse:{name,image:"",stats:{speed:55+Math.floor(Math.random()*35),stamina:55+Math.floor(Math.random()*35),burst:55+Math.floor(Math.random()*35),luck:50+Math.floor(Math.random()*45)}},ready:true,bet:null});
    emitRoom(room);ack({ok:true,room:publicRoom(room)});
  });

  socket.on("bet:place",({code,userId,targetId,amount},ack=()=>{})=>{
    const room=rooms.get(String(code));if(!room||room.status!=="lobby")return ack({ok:false,message:"当前不能竞猜"});const player=findPlayer(room,userId);if(!player)return ack({ok:false,message:"你不在该房间"});
    const value=Math.max(10,Math.min(500,Math.floor(Number(amount||0)/10)*10));if(!room.players.some(p=>p.id===targetId))return ack({ok:false,message:"参赛马不存在"});
    if(player.bet?.amount)addPoints(userId,player.bet.amount);if(!spendPoints(userId,value))return ack({ok:false,message:"积分不足"});player.bet={targetId,amount:value,odds:2};emitRoom(room);ack({ok:true,points:getUser(userId).points,bet:player.bet});
  });

  socket.on("race:start",({code,userId},ack=()=>{})=>{
    const room=rooms.get(String(code));if(!room)return ack({ok:false,message:"房间不存在"});if(room.hostId!==userId)return ack({ok:false,message:"只有房主可以开赛"});if(room.players.length<3)return ack({ok:false,message:"至少 3 人才能比赛"});if(room.status!=="lobby")return ack({ok:false,message:"当前不能开赛"});
    room.status="racing";room.race=buildRacePlan(room.players,room.code);emitRoom(room);io.to(room.code).emit("race:plan",{race:room.race,players:publicRoom(room).players});ack({ok:true});
    const finishDelay=Math.max(0,room.race.startedAt-Date.now())+room.race.durationMs+450;
    setTimeout(()=>finishRace(room.code,room.race.id),finishDelay);
  });

  socket.on("room:reset",({code,userId},ack=()=>{})=>{
    const room=rooms.get(String(code));if(!room)return ack({ok:false,message:"房间不存在"});if(room.hostId!==userId)return ack({ok:false,message:"只有房主可以再开一局"});if(room.status!=="finished")return ack({ok:false,message:"当前不能重置"});
    room.status="lobby";room.race=null;room.players.forEach(p=>p.bet=null);emitRoom(room);ack({ok:true,room:publicRoom(room)});
  });
...
function finishRace(code,raceId){
  const room=rooms.get(code);if(!room||room.status!=="racing"||room.race?.id!==raceId)return;
  room.status="finished";const winnerId=room.race.order[0].playerId;const betResults={};
  for(const player of room.players){if(!player.bet)continue;if(player.bet.targetId===winnerId){const payout=player.bet.amount*player.bet.odds;addPoints(player.id,payout);addPointsWon(player.id,payout-player.bet.amount);betResults[player.id]={won:true,payout};}else betResults[player.id]={won:false,payout:0};}
  const results=room.race.order.map((entry,index)=>({rank:index+1,playerId:entry.playerId,name:room.players.find(p=>p.id===entry.playerId)?.name||"参赛者",timeMs:entry.timeMs}));
  recordRace({roomCode:room.code,raceId:room.race.id,results,winnerId});emitRoom(room);io.to(room.code).emit("race:finish",{results,winnerId,betResults});
}
